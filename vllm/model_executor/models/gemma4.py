...
class Gemma4MoE(nn.Module):
    ...
    def __init__(
        ...
        self.experts = FusedMoE(
            ...
            intermediate_size=getattr(config, "expert_intermediate_size", None) or config.moe_intermediate_size,
            reduce_results=True,
            renormalize=True,
            quant_config=quant_config,
    ...
    def load_weights(
        ...
        def _weight_iterator():
            ...
            for name, weight in weights:
                ...
                if re.search(r'layers\.\d+\.experts\.', name) and '.moe.' not in name:
                    name = re.sub(r'(layers\.\d+\.)experts\.', r'\1moe.', name)
